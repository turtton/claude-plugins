...
質問の前に、以下の観点を確認して不明点を洗い出してください:

- 新規・変更エンティティにリレーションがある場合: 削除時のポリシー（CASCADE / RESTRICT / SET NULL 等）
- 既存 API やインターフェースの型を変更する場合: 他の利用者（別アプリ、他画面等）への影響
- 既存の振る舞いを変更する場合: 後方互換性の要否
...
2. **主要な設計判断**: 重要な判断ごとに以下を明記すること
   ...
   - 検討した代替案（却下した案を含む）
   - 選択理由
3. **影響範囲**: 変更が及ぶファイル・モジュールのリスト（以下のカテゴリで分類）
   - **プロダクションコード**: ソースファイル
   - **テストコード**: テストファイル
   - **インフラ（設定、スキーマ変更、コード生成等）**: 基盤変更
...
#### インフラ（設定、スキーマ変更、コード生成等）

直接的な機能実装以外の変更（設定変更、マイグレーション、依存関係の登録、コード生成の再実行等）。
...
新規実装が模倣すべき既存ファイルとその理由。

#### インフラ変更

直接的な機能実装以外に必要な基盤変更（設定変更、スキーマ変更、コード生成等）。
```
